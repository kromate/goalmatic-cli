...
Store `GOALMATIC_TOKEN` in the CI secret manager and run `goalmatic whoami --json` or `goalmatic status --json`.
...
origin, explicitly set `GOALMATIC_API_URL` too. A repository's `apiOrigin` cannot
redirect an environment token to another host.
